...
Steps to Reproduce: 1. Enter text in search field
2. Press Enter key
3. Verify search functionality
Expected Result: 1. Text entered successfully
2. User navigated to catalog page with search results
3. Search results match the entered text
Actual Result:Text is entered succesfully but the catalog does not provide the match results
...
Steps to Reproduce:1. Fill all required fields
2. Enter invalid postal code: "ABC123"
3. Click "Next" button
4. Observe validation behavior
5. Test other invalid formats
Expected Result: 1. All fields populated correctly
...
Expected Result: 1. All fields populated
...
Notes: The user can stilly manually correct the input and try again,user is affected,order might be delivered to wrong address
...
Notes: User can stilly manually correct the input and try again,affects both users and admins,it might prevent order processing
...
Affected FR(s): FR-004,FR-005
Steps to Reproduce:1. Resize browser to 375×667
2. Navigate through all pages
3. Test all interactions (tap, scroll)
4. Verify layout is responsive
5. Verify text is readable
Expected Result:1. Viewport resized correctly
2. Layout adapts to mobile
3. All features accessible
4. Text readable, touch targets adequate size
5. Responsive design works
Actual Result:Book titles in cart page overlap with the quantinty Field

All Features are accessible and layout adapts correctly to Mobile
Attachments:https://drive.google.com/drive/folders/17OrIxZ3VNbMJVDBH6ZO0klBzGrqn41ci?usp=drive_link
Notes: User can try rotating the phone to landscape or zoom in/out,developers can apply css for compatibilitybook title overlaps the quantiity field creating poor user expirience
